...
@section('content')
<body ng-controller="StyleGuideCtrl" id="style-guide">
	...
	<section>
		...
		<div class="style-guide">
			<form action="" name="styleGuideForm">
				<md-checkbox ng-model="checkboxes.cb0" aria-label="Checkbox">
					Value: @{{ checkboxes.cb0 }}
				</md-checkbox>

				<md-checkbox ng-model="checkboxes.cb1" ng-true-value="'yes'" ng-false-value="'no'" aria-label="Checkbox with custom values">
					Value: @{{ checkboxes.cb1 }}
				</md-checkbox>

				<md-checkbox ng-model="checkboxes.cb2" ng-disabled="true" aria-label="Disabled checkbox">
					Checkbox (Disabled)
				</md-checkbox>

				<md-checkbox ng-model="checkboxes.cb3" ng-init="checkboxes.cb3=true" ng-disabled="true" aria-label="Disabled checked checkbox" >
					Checkbox (Disabled, Checked)
				</md-checkbox>

				<md-checkbox ng-model="checkboxes.cb4" md-no-ink aria-label="Checkbox No Ink">
					Checkbox (No Ink)
				</md-checkbox>
			</form>
		</div>
		...
		<div class="style-guide">
			<form action="" name="styleGuideForm">
				<md-radio-group ng-model="user.gender" layout layout-sm="column">
		...
		<div class="style-guide">
			<form action="" name="styleGuideForm">
				<div layout layout-sm="column">
					<md-switch flex  ng-model="switches.sw0" ng-true-value="'Yes I do want to collaborate with others'" ng-false-value="'No I don&rsquo;t want to collaborate with others'" aria-label="Switch 1">
						@{{ switches.sw0 }}
					</md-switch>
				</div>
			</form>
		...
		<div class="style-guide">
			<form action="" name="styleGuideForm">
				<md-select ng-model="goals" placeholder="Categories">
					<md-optgroup label="Category A">
						<md-option ng-value="goal.name" ng-repeat="goal in goals | filter: {category: 'Category A' }">@{{ goal.name }}</md-option>
					</md-optgroup>
					<md-optgroup label="Category B">
						<md-option ng-value="goal.name" ng-repeat="goal in goals | filter: {category: 'Category B' }">@{{ goal.name }}</md-option>
					</md-optgroup>
				</md-select>
			</form>
		</div>
			...
			<form action="" name="styleGuideForm">
				...
				<div layout layout-sm="column">
					<md-input-container flex>
						...
						<input ng-model="user.mobilePhone">
					</md-input-container>
					<md-switch flex ng-model="data.sw1" ng-true-value="'public'" ng-false-value="'private'"  aria-label="Switch 1">
						@{{ data.sw1 }}
					</md-switch>
				</div>
				...
				<div layout layout-sm="column">
					...
					<md-switch flex ng-model="data.sw2" ng-true-value="'public'" ng-false-value="'private'"  aria-label="Switch 1">
						@{{ data.sw2 }}
					</md-switch>
				</div>
				...
				<div layout layout-sm="column">
					<md-input-container flex>
						<label>Street</label>
						<input ng-model="company.address.postalCode" name="postalCode" required>
					</md-input-container>
				</div>
				...
				<div layout layout-sm="column">
					...
					<md-input-container flex>
						...
						<input ng-model="company.address.city" name="Locality" required>
					</md-input-container>
				</div>
